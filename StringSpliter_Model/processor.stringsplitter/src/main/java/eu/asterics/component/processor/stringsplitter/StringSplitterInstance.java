...
public class StringSplitterInstance extends AbstractRuntimeComponentInstance {
    ...
    // *****************extended by Berenice*******************

    final IRuntimeOutputPort opOut17 = new DefaultRuntimeOutputPort();
    final IRuntimeOutputPort opOut18 = new DefaultRuntimeOutputPort();
    final IRuntimeOutputPort opOut19 = new DefaultRuntimeOutputPort();
    final IRuntimeOutputPort opOut20 = new DefaultRuntimeOutputPort();
    //*********************end Berenice*******************************
    ...
    // *********** Berenice*********
    int propStartIndex = 20;  // separet de string in two parts, division begins in string number 20
    // ***********end  Berenice*********
    ...
    public IRuntimeOutputPort getOutputPort(String portID) {
        ...
        // **************Berenice***********
        if ("out17".equalsIgnoreCase(portID)) {
            return opOut13;
        }
        if ("out18".equalsIgnoreCase(portID)) {
            return opOut14;
        }
        if ("out19".equalsIgnoreCase(portID)) {
            return opOut15;
        }
        if ("out20".equalsIgnoreCase(portID)) {
            return opOut16;
        }
        //*********end Berenice*************** */
    ...
    private final IRuntimeInputPort ipInput = new DefaultRuntimeInputPort() {
        ...
        public void receiveData(byte[] data) {
            ...
            if(propStartIndex == 0){  // breaks the String in two parts
                 //for (int i = 0; i < stringparts.length; i++) {
                for (int i = 0; i < stringparts.length; i++) {    //************Berenice read until string 19 */
                    switch (i) {
                        case 0:
                        ...
                        case 15:
                            ...
                            //***********Berenice********* */
                        case 16:
                            opOut17.sendData(ConversionUtils.stringToBytes(stringparts[i]));
                            break;
                        case 17:
                            opOut18.sendData(ConversionUtils.stringToBytes(stringparts[i]));
                            break;
                        case 18:
                            opOut19.sendData(ConversionUtils.stringToBytes(stringparts[i]));
                            break;
                        case 19:
                            opOut20.sendData(ConversionUtils.stringToBytes(stringparts[i]));
                            break;    
                    }
                }
            }
